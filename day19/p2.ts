...
const work = (BP: Blueprint, TIME: number) => {
  ...
  const maxObsidianMps = BP.geodeBot.obs;
...
const notEnoughMinerals = (readlines: string[], length: number) => {
  const blueprints = parseBlueprint(readlines);

  let multiplyTogether = 1;

  for (let i = 0; i < length; i++) {
    const maxGeodes = work(blueprints[i], 32);
    multiplyTogether *= maxGeodes;
  }

  return multiplyTogether;
};

(async () => {
  // const example = await readFromFile("./example.txt", true);
  // console.log(notEnoughMinerals(example, 2)); // 56 * 62 = 3472

  const input = await readFromFile("./input.txt");
  console.log(notEnoughMinerals(input, 3)); // 7350
})();
