...
const traversePath = (
  ...
  for (let i = 0; i < follows.length; i++) {
    ...
    const { nextPos, nextFacing } = followLine(
      map,
      currentPos,
      +follows[i],
      facing,
  ...
  return { cPos: currentPos, facing };
};

const monkeyMap = (readGroups: string[], surfaceLen: number) => {
  ...
  const { cPos, facing } = traversePath(map, follows, connectMap, surfaceLen);
...
(async () => {
  ...
  console.log(monkeyMap(input, 50)); // 127012
})();
