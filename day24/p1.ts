...
const getNextBlizzard = (blizzards: Blizzard[], gridLength: Pos) => {
  const [rowLength, colLength] = gridLength;
...
const blizzardBasin = (readlines: string[]) => {
  ...
  return bfs(grid, blizzards, [0, 1], [rowLength - 1, colLength - 2], modTime);
};

(async () => {
  const exampleSmall = await readFromFile("./example-small.txt");
  console.log(blizzardBasin(exampleSmall)); // 10

  const example = await readFromFile("./example.txt");
  console.log(blizzardBasin(example)); // 18

  const input = await readFromFile("./input.txt");
  console.log(blizzardBasin(input)); // 322
})();
