...
const printVisualize = async (row: number, col: number, step: number) => {
  if (visualGrid[row][col] === "a") {
    visualGrid[row][col] = `\x1b[30;107m${visualGrid[row][col]}\x1b[0m`;
  } else {
    visualGrid[row][
      col
...
(async () => {
  ...
  // visualWait = 100;
  // console.log(await hillClimbingAlgorithm(example)); // 29

  const input = await readFromFile("./input.txt");
  visualWait = 0;
  console.log(await hillClimbingAlgorithm(input)); // 512
})();
